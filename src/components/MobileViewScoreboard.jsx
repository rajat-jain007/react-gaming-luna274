import React from "react";

function MobileViewScoreboard() {
  return (
    <div className="grid grid-cols-1 gap-4 md:hidden bg-white ">
      {/* Table 1 */}
      <div className="rounded-lg shadow-md">
        <div className="text-white bg-red-700 text-sm font-semibold p-1">
          Cricket
        </div>
        <div className="text-black bg-slate-100 flex flex-col text-base font-semibold ">
          South Africa v Australia
          <span className="text-sm font-normal">
            10 Sep 01:00 | Women's Caribbean Premier League{" "}
          </span>
        </div>

        <div className="flex grid-cols-6 bg-slate-100 gap-1 w-full mb-3">
          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">1.60</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.55</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.75</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.80</div>
        </div>

        <div className="text-black bg-slate-100 flex flex-col text-base font-semibold ">
          Guyana Warriors W v Trinbago Knight Riders W
          <span className="text-sm font-normal">
            10 Sep 01:00 | Caribbean Premier League League
          </span>
        </div>

        <div className="flex grid-cols-6 bg-slate-100 gap-1 w-full mb-3  ">
          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">1.60</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.55</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.75</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.80</div>
        </div>

        <div className="text-black bg-slate-100 flex flex-col text-base font-semibold ">
          Pakistan v India
          <span className="text-sm font-normal">10 Sep 01:00 | Asia Cup</span>
        </div>

        <div className="flex grid-cols-6 bg-slate-100 gap-1 w-full mb-3  ">
          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">1.60</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.55</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.75</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.80</div>
        </div>

        <div className="text-black bg-slate-100 flex flex-col text-base font-semibold  ">
          England v New Zealand
          <span className="text-sm font-normal">
            10 Sep 01:00 | One Day Internationals
          </span>
        </div>

        <div className="flex grid-cols-6 bg-slate-100 gap-1 w-full mb-3  ">
          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">1.60</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.55</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.75</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.80</div>
        </div>
      </div>

      {/* Table 2 */}
      <div className="rounded-lg shadow-md">
        <div className="text-white bg-red-700 text-sm font-semibold p-1">
          Soccer
        </div>
        <div className="text-black bg-slate-100 flex flex-col text-base font-semibold ">
          Oman U23 v Jordan U23
          <span className="text-sm font-normal">
            10 Sep 01:00 | AFC U23 Championship
          </span>
        </div>

        <div className="flex grid-cols-6 bg-slate-100 gap-1 w-full mb-3  ">
          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">1.60</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.55</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.75</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.80</div>
        </div>

        <div className="text-black bg-slate-100 flex flex-col text-base font-semibold ">
          Anderlecht (W) v SK Brann (W)
          <span className="text-sm font-normal">
            22 Sep 01:00 | UEFA Women's Champions League
          </span>
        </div>

        <div className="flex grid-cols-6 bg-slate-100 gap-1 w-full mb-3  ">
          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">1.60</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.55</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.75</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.80</div>
        </div>

        <div className="text-black bg-slate-100 flex flex-col text-base font-semibold ">
          CA Independiente v Estudiantes
          <span className="text-sm font-normal">
            09 Sep 01:00 | Argentinian Cup
          </span>
        </div>

        <div className="flex grid-cols-6 bg-slate-100 gap-1 w-full mb-3  ">
          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">1.60</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.55</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.75</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.80</div>
        </div>

        <div className="text-black bg-slate-100 flex flex-col text-base font-semibold ">
          Comoros v Zambia
          <span className="text-sm font-normal">
            10 Sep 01:00 | CAF African Nations Cup Qualifiers
          </span>
        </div>

        <div className="flex grid-cols-6 bg-slate-100 gap-1 w-full mb-3  ">
          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">1.60</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.55</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.75</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.80</div>
        </div>

        <div className="text-black bg-slate-100 flex flex-col text-base font-semibold ">
          Oman U23 v Jordan U23
          <span className="text-sm font-normal">
            10 Sep 01:00 | AFC U23 Championship
          </span>
        </div>

        <div className="flex grid-cols-6 bg-slate-100 gap-1 w-full mb-3  ">
          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">1.60</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.55</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.75</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.80</div>
        </div>
      </div>
      {/* Table 3 */}

      <div className="rounded-lg shadow-md">
        <div className="text-white bg-red-700 text-sm font-semibold p-1">
          Tennis
        </div>
        <div className="text-black bg-slate-100 flex flex-col text-base font-semibold ">
          A. Cornet v T Zidansek
          <span className="text-sm font-normal">
            15 Sep 01:00 | WTA Bari 2023
          </span>
        </div>

        <div className="flex grid-cols-6 bg-slate-100 gap-1 w-full mb-3  ">
          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">1.60</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.55</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.75</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.80</div>
        </div>

        <div className="text-black bg-slate-100 flex flex-col text-base font-semibold ">
          K Siniakova v Zarazua
          <span className="text-sm font-normal">
            09 Sep 01:00 | WTA San Diego 2023
          </span>
        </div>

        <div className="flex grid-cols-6 bg-slate-100 gap-1 w-full mb-3  ">
          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">1.60</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.55</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.75</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.80</div>
        </div>

        <div className="text-black bg-slate-100 flex flex-col text-base font-semibold ">
          A Sasnovich v Er Cayetano
          <span className="text-sm font-normal">
            10 Sep 01:00 | WTA San Diego 2023
          </span>
        </div>

        <div className="flex grid-cols-6 bg-slate-100 gap-1 w-full mb-3  ">
          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">1.60</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.55</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.75</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.80</div>
        </div>

        <div className="text-black bg-slate-100 flex flex-col text-base font-semibold ">
          Gauff v A Sabalenka
          <span className="text-sm font-normal">
            10 Sep 01:00 | Women's US Open 2023
          </span>
        </div>

        <div className="flex grid-cols-6 bg-slate-100 gap-1 w-full mb-3  ">
          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">1.60</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.55</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.75</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.80</div>
        </div>

        <div className="text-black bg-slate-100 flex flex-col text-base font-semibold ">
          C Liu v Aly Ahn
          <span className="text-sm font-normal">
            10 Sep 01:00 | WTA San Diego 2023
          </span>
        </div>

        <div className="flex grid-cols-6 bg-slate-100 gap-1 w-full mb-3  ">
          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">1.60</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.55</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.75</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.80</div>
        </div>

        <div className="text-black bg-slate-100 flex flex-col text-base font-semibold ">
          Camila Osorio v Loeb
          <span className="text-sm font-normal">
            10 Sep 01:00 | WTA San Diego 2023
          </span>
        </div>

        <div className="flex grid-cols-6 bg-slate-100 gap-1 w-full mb-3  ">
          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">1.60</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.55</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.75</div>

          <div className="grid-cols-1  bg-sky-400 px-4 mt-2 ">2.60</div>
          <div className="grid-cols-1  bg-green-300 px-4 mt-2 ">2.80</div>
        </div>
      </div>
    </div>
  );
}

export default MobileViewScoreboard;
